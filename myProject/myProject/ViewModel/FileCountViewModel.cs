...
{
    public class FileCountViewModel : Observable
    {
        private List<File> files;
        public FileCountViewModel(List<File> list)
        {
            if (list != null)
            {
                files = new List<File> { };
                ...
                {
                    file.Name = file.Files + " " + file.Path;
                    FileCountName(file.AllFiles);
                }
            }
        }
        private void FileCountName(List<File> file)
        {
            foreach (var f in file)
            {
                f.Name = f.Files + " " + f.Name;
                if (f.AllFiles != null)
                {
                    FileCountName(f.AllFiles);
                }
            }
        }
